...
function buildOWMOneCallUrl(float $lat, float $lon): string
{
  $params = [
    ...
    'appid' => OWM_API_KEY,
    'units' => 'metric',
    // CHANGE: keep daily so we can derive sunrise/sunset parity
...
function transformOWMToOpenMeteo(float $lat, float $lon, array $owm, int $hours, string $timeformat): array
{
  ...
  // ---- Daily sunrise/sunset shaping ----
  $dailyTime = [];
  $dailySunrise = [];
  $dailySunset = [];

  if (isset($owm['daily']) && is_array($owm['daily'])) {
    foreach ($owm['daily'] as $d) {
      $dt = isset($d['dt']) ? (int)$d['dt'] : null;
      if ($dt === null) continue;

      $sr = isset($d['sunrise']) ? (int)$d['sunrise'] : null;
      $ss = isset($d['sunset']) ? (int)$d['sunset'] : null;

      $dtLocal = $dt + $tzOffset;
      $srLocal = ($sr !== null) ? $sr + $tzOffset : null;
      $ssLocal = ($ss !== null) ? $ss + $tzOffset : null;

      if ($timeformat === 'unixtime') {
        $dailyTime[] = $dtLocal;
        $dailySunrise[] = $srLocal ?? 0;
        $dailySunset[] = $ssLocal ?? 0;
      } else {
        $dailyTime[] = gmdate('Y-m-d', $dtLocal);
        $dailySunrise[] = ($srLocal !== null) ? (gmdate('Y-m-d\TH:i:s', $srLocal) . offsetToIsoSuffix($tzOffset)) : null;
        $dailySunset[]  = ($ssLocal !== null) ? (gmdate('Y-m-d\TH:i:s', $ssLocal) . offsetToIsoSuffix($tzOffset)) : null;
      }
    }
  }
  ...
  return [
    ...
    'daily_units' => [
      'time' => ($timeformat === 'unixtime') ? 'unixtime' : 'iso8601',
      'sunrise' => ($timeformat === 'unixtime') ? 'unixtime' : 'iso8601',
      'sunset'  => ($timeformat === 'unixtime') ? 'unixtime' : 'iso8601',
    ],
    'daily' => [
      'time' => $dailyTime,
...
function writeExpectedStateToRainglowData($sql, float $lat = 51.42, float $lon = -1.73): bool
{
  ...
  $times = $om['hourly']['time']; // local unixtime when timeformat=unixtime + timezone=auto
  ...
  $currentBlockIdx = max(0, min(5, $currentBlockIdx));
  ...
  for ($i = 0; $i < 6; $i++) {
    ...
    if (!empty($idxs)) {
      foreach ($idxs as $k) {
        ...
        $pp = isset($prob[$k]) ? (float)$prob[$k] : 0.0; // already %
    ...
    $segments[$i] = [
      ...
      'probRain'  => $maxProb,
      'rainMm'    => $maxRainMm,
      'timeStart' => substr($timeStartStr, 0, 5),
      'timeEnd'   => substr($timeStartStr, 6, 5),
      'color'     => $rgb,
    ];
  }

  // Optional: definitely rainy now -> white
  ...
  for ($i = 0; $i < 6; $i++) {
    ...
    $values[] = (string)$seg['timeStart'];
    $values[] = (string)$seg['timeEnd'];
...
function segmentToRgb(float $rainMm, float $probRainPct): array
{
  ...
  if ($rainMm < 0.5) {
    $dry = 100.0 - $probRainPct;
    $g = (int)round(scale($dry, 0.0, 100.0, 40.0, 255.0));
    return ['r' => 0, 'g' => $g, 'b' => 0];
  }

  $t = 0.0;
  if ($rainMm <= 2.0)      $t = 0.0;
  else if ($rainMm <= 5.0) $t = 0.5;
  else                     $t = 1.0;
