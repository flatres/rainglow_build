     ...
     private function courseSearch($userId, $search, $limit = null, $offset = null) {
      ...
      switch ($search['sortField']) {
        case 'duration':
          ...
          break;
        case 'title':
          $orderString = "ORDER BY {$search['sortField']} {$search['sortDirection']}";
          break;
        case 'cost':
      ...
      if (strlen($text)>1) {
        ...
        $whereString = "EXISTS (
          SELECT 1 FROM courses_profiles cp2
          WHERE cp2.courseId = c.id
          AND MATCH(cp2.title, cp2.description, cp2.outcomes) AGAINST (? IN NATURAL LANGUAGE MODE)
        ) AND ";
        ...
        //  $whereString = '';
        $queryData = [$text, $categoryId, $countryId];
    ...
    public function coursesSearchPost($request, $response, $args) {
     ...
     public function getCourseById($id) {
        $course = $this->sql->query(
            " SELECT DISTINCT c.id, c.providerId, c.isOnline, c.cost, c.isLive, c.currencyId, DATE_FORMAT(c.startDate, '%d-%m-%Y') startDateFormatted, c.duration, c.durationUnitId, c.location, c.countryId, p.logoImg, p.name as provider, ap.membershipExpires
              FROM courses c
              LEFT JOIN courses_profiles cp ON c.id = cp.courseId
              LEFT JOIN categories_courses cc ON c.id = cc.courseId
              LEFT JOIN providers p ON c.providerId = p.id
